...
<nav class="flex justify-center sm:justify-end items-center space-x-8 text-sm">
    <?php if ($arResult["FORM_TYPE"] == "login") :?>
        <a class="text-gray-500 hover:text-orange" href="<?=$arResult['AUTH_REGISTER_URL']?>">
        ...
        <a class="text-gray-500 hover:text-orange" href="<?=$arParams['REGISTER_URL'] . ((! empty($arResult['BACKURL']) && stripos($arResult['BACKURL'], 'backurl=') === false) ? '?backurl=' . $arResult['BACKURL'] : '')?>">
    ...
    <?php else :?>
        <a class="text-black hover:text-orange" href="<?=$arParams['PERSONAL_DATA']?>">
            ...
            <span class="inline-block font-bold"><?=GetMessage('GREETINGS') . ', ' . $arResult["USER_NAME"] . '!'?></span>
        </a>
        <a class="text-gray-500 hover:text-orange" href="<?=$arParams['PROFILE_URL']?>">
            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block text-orange h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            <?=GetMessage('AUTH_PROFILE')?>
        </a>
        <div class="text-gray-500 hover:text-orange">
            <form action="<?=$arParams['REDIRECT_URL']?>">
                <svg xmlns="http://www.w3.org/2000/svg" class="inline-block text-orange h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
